...
import type { CardSession, CardSessionOutcome } from "@/lib/arbitrage/weightedCardData";
...
export function CardSessionModal({ cardName, stackSize, defaultBuyPrice, outcomes, onSubmit, onClose }: Props) {
  ...
  const submit = () => {
    if (stacksProcessed <= 0) return;
    const sessionOutcomes: CardSessionOutcome[] = rows
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Log a session for ${cardName}`}
      >
        <div className="mb-3 flex items-start justify-between">
          <div>
            <h3 className="text-base font-semibold">Log a session - {cardName}</h3>
            <p className="text-xs text-[var(--muted)]">
              Enter how many of each outcome you got this session. Rates below are pre-filled from live
              prices - edit them if that isn&apos;t what you actually paid/sold for.
            </p>
          </div>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] text-left text-xs text-[var(--muted)]">
                ...
                <th className="px-2 py-1 font-medium">Times pulled</th>
                <th className="px-2 py-1 font-medium">Value per pull (chaos)</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.rewardName} className="border-b border-[var(--border)] last:border-0">
                  <td className="px-2 py-1">
                    {r.rewardName}
                    {r.rewardQuantity > 1 && <span className="text-xs text-[var(--muted)]"> x{r.rewardQuantity}</span>}
                  </td>
                  <td className="px-2 py-1">
                    <input
                      type="number"
                      min={0}
                      value={r.timesPulled}
                      onChange={(e) => updateRow(r.rewardName, { timesPulled: Math.max(0, Number(e.target.value) || 0) })}
